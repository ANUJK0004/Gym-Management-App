...
const REPORT_FROM_EMAIL =
  defineSecret("REPORT_FROM_EMAIL");
...
export const exportReport = onCall(
  ...
  async (request) => {
    ...
    const data =
      request.data as Partial<ExportRequest>;
    ...
    if (ownerData?.role !== "owner") {
      throw new HttpsError(
        "permission-denied",
        "Only gym owners can export reports.",
      );
    }

    const gymId =
      String(ownerData.gymId ?? "")
        .trim();

    if (!gymId) {
      throw new HttpsError(
        "failed-precondition",
        "The owner is not associated with a gym.",
      );
    }
    ...
    const fileName =
      `${safeType}_${timestamp}.${extension}`;

    const storagePath =
      `reports/${gymId}/${fileName}`;
    ...
    const exportRef =
      db.collection("reportExports").doc();
...
async function getNpsResponses(
  gymId: string,
): Promise<Record<string, any>[]> {
  try {
    const snapshot =
        await db.collection("npsResponses")
          .where(
            "gymId",
            "==",
            gymId,
  ...
  switch (period) {
    ...
    case "lastMonth": {
      ...
      return {
        ...
        label:
            `Last Month · ${monthName(start.getMonth())} ${start.getFullYear()}`,
      };
    }
    ...
    case "lastQuarter": {
      ...
      return {
        ...
        label:
            `Q${currentQuarter === 0 ? 4 : currentQuarter} ${
              start.getFullYear()
            }`,
      };
    }
...
function buildReportData({
  ...
  npsResponses: Record<string, any>[];
}) {
  const sectionSet =
      new Set(
    sections,
...
async function sendReportEmail({
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject:
        `SweatSync ${reportType} report · ${periodLabel}`,
    text:
        `Your SweatSync report is ready.\n\n` +
        `Report: ${reportType}\n` +
        `Period: ${periodLabel}\n` +
        `File: ${fileName}\n\n` +
        `Download it here:\n${downloadUrl}\n\n` +
...
function parseFormat(
  ...
  return parsed as ExportFormat;
}

function parsePeriod(
  value: unknown,
): ExportPeriod {
  const valid: ExportPeriod[] = [
    "thisWeek",
    "thisMonth",
    "lastMonth",
    "thisQuarter",
