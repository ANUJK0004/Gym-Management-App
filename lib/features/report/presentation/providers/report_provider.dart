...
FutureProvider<List<ReportTrend>>(
      (ref) async {
    final gym =
    await ref.watch(
      ownerGymProvider.future,
...
FutureProvider<List<ReportPeakHour>>(
      (ref) async {
    final gym =
    await ref.watch(
      ownerGymProvider.future,
...
FutureProvider<
    List<ReportMembershipBreakdown>>(
      (ref) async {
    final gym =
    await ref.watch(
      ownerGymProvider.future,
...
FutureProvider<List<DownloadableReport>>(
      (ref) async {
    return ref
        .watch(reportRepositoryProvider)
...
FutureProvider<ReportDashboardData>(
      (ref) async {
    final results = await Future.wait([
      ...
      ref.watch(
        reportTrendsProvider.future,
      ),
      ref.watch(
        reportPeakHoursProvider.future,
      ),
      ref.watch(
        reportMembershipBreakdownProvider.future,
      ),
      ref.watch(
        downloadableReportsProvider.future,
      ),
    ]);

    return ReportDashboardData(
      summary:
      results[0] as ReportSummary,
      trends:
      results[1] as List<ReportTrend>,
      peakHours:
      results[2] as List<ReportPeakHour>,
      membershipBreakdown:
      results[3]
      as List<ReportMembershipBreakdown>,
      downloadableReports:
      results[4]
      as List<DownloadableReport>,
    );
  },
);
...
class ReportExportController
    extends AsyncNotifier<
        ReportExportResult?> {
  @override
  Future<ReportExportResult?> build() async {
    return null;
  }
